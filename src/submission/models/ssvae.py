...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        kl_y = ut.kl_cat(y_prob, y_logprob, torch.log(torch.tensor(1.0 / self.y_dim)))
        kl_y = kl_y.mean() 

        kl_z = ut.kl_normal(qm, qv, pm, pv)
        kl_z = kl_z.view(self.y_dim, -1)
        kl_z = (y_prob.t() * kl_z).sum(dim=0).mean() 

        logits = self.dec(z, y)
        rec = ut.log_bernoulli_with_logits(x, logits)
        rec = rec.view(self.y_dim, -1)
        rec = -(y_prob.t() * rec).sum(dim=0).mean() 
