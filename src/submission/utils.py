...
def log_normal(x, m, v):
    ...
    ################################################################################
    ### START CODE HERE ###
    log_const = -0.5 * torch.log(torch.tensor(2 * torch.pi))
    log_prob = log_const - 0.5 * torch.log(v) - 0.5 * ((x - m) ** 2) / v
...
def log_normal_mixture(z, m, v):
    ...
    z = z.unsqueeze(1)
    log_const = -0.5 * torch.log(torch.tensor(2 * torch.pi))
    log_prob = log_const - 0.5 * torch.log(v) - 0.5 * ((z - m) ** 2) / v

    log_prob = log_prob.sum(dim=-1)

    log_prob = torch.logsumexp(log_prob, dim=1) - torch.log(torch.tensor(m.size(1)))
    ### END CODE HERE ###
    ################################################################################
    # End of code modification
    ################################################################################
